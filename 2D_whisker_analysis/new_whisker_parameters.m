...
% step4: on croppted whisker image, find the whisker base and tip position by pressing enter key 

% all data are save on the SealWhisker.mat file, if you want to delte the
% most recent row, please use deletRow.m file 
close all
clear all
warning off
nBar = 11; 
...
xlabel('index numbers')
ylabel('sum of white pixel numbers')
grid on
%% find the convex and concave points on centerline
...
for i = 2 : 1: length(left_convex_concave_xx)-1
   ...
   if ((k1 > 0.02) || (k2 > 0.02)) && ((current_height_left < 1.8 * mid_height) &&  (current_height_right < 1.8 * mid_height)) && ((current_height_right > 0.15* mid_height) && (current_height_left > 0.15* mid_height))     
       if left_count3 ~= 0 
           if abs(m1 - m) ~= 1
               left_count3 = 0;
           end 
...
for i = 2 : 1: length(right_convex_concave_xx)-1
    ...
    mid_point = round(length(right_convex_concave_yy)/2);
    mid_height = abs(right_convex_concave_yy(mid_point) - right_convex_concave_yy(mid_point + 1));
    ...
    if ((k1 > 0.01) || (k2 > 0.01))  && ((current_height_left < 1.8 * mid_height) &&  (current_height_right < 1.8 * mid_height)) && ((current_height_right > 0.15* mid_height) && (current_height_left > 0.15* mid_height)) 
       if right_count3 ~= 0 
           if abs(m1 - m) ~= 1
               right_count3 = 0;
           end 
...
% Check if the whisker is valid, it would automaticaly save as valid, if
% user think it is not valid whisker, then they need to change valid number
% from 1 to 0 directly under the SealWhisker.mat file 
ma.IfValid(a(1)+1, 1) = 1;
clear ma;
