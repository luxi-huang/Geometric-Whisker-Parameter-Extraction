...
im = imcrop(im0);
%% crop the background and get the background color 
% im_background = imcrop(im0);
% Ig = im_background(:,:,2);
% Gave = uint8(mean(Ig));
% disp(length(Gave));
% fprintf('background g value %d,\n',Gave);
%%
close
...
[xc, yc] = ginput();
zoom OUT
xc = round(xc(end)); yc = round(yc(end));
plot(xc,yc,'ro');

%% find the mask of image 
if check_gray == 0 
...
%% Generate center_line_1 whisker (base on the concentration of black color )
radius = 150;
...
%% cancell small white and black areas
if check_gray == 0
    % rgb image
...
% title('Objects with boundaries overlaid', 'FontSize', 25);

%% Generate center_line_1 on blured image whisker (base on the concentration of black color )
radius = 150;
x = zeros(abs(yb-yt)+1,1);
y = zeros(abs(yb-yt)+1,1);
center = xb;
...
%% plot smooth the lines 
subplot(1,3,3);
imshow(binaryImage);
hold on;
plot(x,y,'b-'); 
%% find new centerlines
% step 1: find the smooth line of original centerline 
% step 2: find the perpendicular lines of the centerline
% step 3: find the white pixel numbers alonge the perpendicualr lines in lower/upper (left/right) directions 
% step 4: get the upper/lower (left/right) outliens  
% step 5: find the new center lines based on the mid posiiton of the
...
for j = 1:2:length(y)-10
    ...
    % find the white pixel sum values on the lower side of perpendicualr line 
    ...
    sum_value1 =0;
    % find the white pixel sum values on the upper side of perpendicualr line 
...
fprintf('count');
disp(count);
%% plot new centerlines:
f4 = figure('Position',[100 100 900 800]);
subplot(1,3,1);
imshow(binaryImage);
hold on;
...
imshow(binaryImage);
hold on;
plot(a1,b1,'r-',x(1),y(1),'co');
plot(x,y,'b-'); 
plot(new_center_line_x, new_center_line_y,'k');

%% edge
[B,L] = bwboundaries(binaryImage);
f5 = figure('Position',[100 100 900 800]);
subplot(1,3,1);
imshow(im);
hold on
plot(new_center_line_x, new_center_line_y,'k');
...
fprintf('length of B: %n',length(B));
...
% title('Objects with boundaries overlaid', 'FontSize', 25);
%% plot new center line and concave and convex point
% x_value = 1:10000;
...
%
plot(array_value_smooth,'k');
plot(concave_xx , concave_yy, 'go');
plot(convex_xx , convex_yy, 'ro');
plot(final_convex_concave_xx , final_convex_concave_yy, 'r*');
grid on

%% plot left line and concave and convex point
% x_value = 1:10000;
fig7 = figure('Position',[1050,100,600,400]); hold on
% plot(x_value(1:numel(y_value)));
...
left_convex_xx = [];
left_convex_yy = [];
left_final_concave_xx = []; 
left_final_concave_yy = [];
left_final_convex_xx = [];
left_final_convex_yy = [];
...
for i = 2 : 1: length(left_convex_concave_xx)-1
    ...
    k2 =abs((left_convex_concave_yy(i) - left_convex_concave_yy(i+1))/(left_convex_concave_xx(i) - left_convex_concave_xx(i+1)));
    m = i; 
    mid_length = round(length(left_convex_concave_yy)/2);
    mid_height = abs(left_convex_concave_yy(mid_length) - left_convex_concave_yy(mid_length + 1));
...
%% plot right line and concave and convex point
% x_value = 1:10000;
fig8 = figure('Position',[1050,100,600,400]); hold on
% plot(x_value(1:numel(y_value)));
...
right_convex_xx = [];
right_convex_yy = [];
right_final_concave_xx = []; 
right_final_concave_yy = [];
right_final_convex_xx = [];
right_final_convex_yy = [];
...
%% get left and right peak posiiton on the whiskers
